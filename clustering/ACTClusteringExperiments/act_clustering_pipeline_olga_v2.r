...
#doHC = F    
    ...
    cleanDF <- cleanDF %>%
      mutate(
        treatmentDurationMins = treatmentDuration/60
      )
...
OutlierFunc1 <- function(clusterDF, kFeatureNames) {
...
####     Clustering with Duration and Adherance features 
Experiment1 <- function() {
    featureNames <- c("patientId", "actDate", 
                    ...
                    "treatmentRollWeekAdherenceScore",
                    "relativeTreatmentDuration")
    cRes <- ClusterWithViz(actTableNames, featureNames, OutlierFunc1, removeOutliers = T, doTendency=F, doKMeans = T, doHC = F, nClusters = 3)

    OnePatientCluster(cRes$clusterDF, cRes$pca, cRes$umapScaled, cRes$kpcaTF, cRes$uPcaDf, cRes$kumapFF, cRes$uUmapDf)
}

###      Breath duration (without amplitude)
Experiment2 <- function() {
    ...
    # Using the same outlier func as in experiment 1                
    cRes <- ClusterWithViz(actTableNames, featureNames, OutlierFunc1, removeOutliers = T, doTendency=F, doKMeans = T, doHC = F, nClusters = 5)
...
Experiment3 <- function() {
    ...
    cRes <- ClusterWithViz(actTableNames, featureNames, OutlierFunc3, removeOutliers = T, doTendency=T, doKMeans = T, doHC = T, nClusters = 4)
...
#tableNameOut <- "act_kmeans_exp_viz"
#tableNameCentroids <- "act_kmeans_centroids_viz"
#ValidateOutputables(tableNameOut, tableNameCentroids)
