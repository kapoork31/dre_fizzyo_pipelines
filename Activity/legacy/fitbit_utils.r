...
FeaturiseFitbitCombined <- function(df) {
  ...
  df %>%
    ...
    mutate(RegressionMVPAThreshold = activeMinutesRegThresh(unique(userid),unique(date)))%>%
    ...
    mutate(activeMinsHrSteps0 = map_dbl(data,
    ...
    mutate(stepNormModerate = map_dbl(data,
           ...
           stepNormNonModerate = map_dbl(data,
                                      StepNormActivityPeriods,
                                      filterExpr = "value_hr < 120")) %>%
