...
source("~/scripts/ACT_v3/1_featurise/utils/act_schema_utils_kunal.R")
source("~/scripts/ACT_v3/1_featurise/utils/act_cleaning_utils_kunal.R")
source("~/scripts/ACT_v3/1_featurise/utils/act_labelling_utils_kunal.R")
source("~/scripts/ACT_v3/1_featurise/utils/act_featurisation_utils_kunal.R")
...
startDate, endDate,devicesData,rawMetaTableName,cleanMetaTableName) {
    ...
    writeLines(pid)
    prMetadata <- tbl(conn, rawMetaTableName) %>%
        ...
        distinct() %>%
        collect()
    if (nrow(prMetadata) == 0) {
        writeLines(sprintf("No data for patient %s exists", pid))
        return ()
    }
    
    datesToClean = sort(prMetadata$date)
    
    if (dbExistsTable(conn, outputTableName)) {
        processedDays <- tbl(conn, cleanMetaTableName) %>%
            filter(patient_record_id == pid &
                date >= startDate &
                date < endDate) %>%
            select(date) %>%
    ...
    if(length(remainingDays) == 0){
        ...
        for (i in 1:(length(remainingDays))) {
            ...
            df = data.frame(pid,startDate)
            colnames(df) = c('patient_record_id','date')
...
startDate, endDate,devicesData, rawActTableName, metaActTableName,labelMetaTableName) {
    ...
    #pid = 'd92bef52-f7ca-4796-8efd-8bee7afd3391'
    
    #print(metaActTableName)
    #print(labelMetaTableName)
    ...
    if(length(remainingSessions) == 0){
        ...
        df <- labelData %>%
            mutate(date = as.Date(time[1])) %>%
            select(date,patientId,sessionId) %>%
            group_by(date, patientId,sessionId) %>%
            summarize()%>%
            collect()
            
        print(nrow(data))
        print(nrow(labelData))
...
FeaturiseStep <- function(conn, patient_Id, inputTableName, outputTableName,startDate, endDate,devicesData) {
