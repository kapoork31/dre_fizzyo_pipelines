...
source("~/scripts/ACT_v3/act_pipeline/src/utils/act_labelling_utils_kunal_1.R")
source("~/scripts/ACT_v3/act_pipeline/src/utils/act_featurisation_utils_kunal.R")
...
startDate, endDate,devicesData, rawActTableName, metaActTableName,labelMetaTableName,outputTableNameMeta) {
    print(pid)
...
FeaturiseStep <- function(conn, patient_Id, inputTableName, outputTableName,startDate, endDate,devicesData,label_treatment_meta,
  ...
  if (dbExistsTable(conn, output_meta)) {
    processedIds <- tbl(conn, output_meta) %>%
  ...
  ValidateDataSchema(data, LabelledACTDataSchema)
 
  #data = xap.read_table('act_laballed_data_test')
  #labelledACTData = data 
  meta_treatments <- tbl(conn, label_treatment_meta) %>%
    filter(patientId == patient_Id & treatmentId %in% ToProcessedIds) %>%
    collect() %>%
    mutate(actDate = as.Date(sessionStart)) %>%
  ...
  dbWriteTable(conn, output_meta, meta_treatments, append = TRUE, row.names = FALSE)
